...
- A- A tagged GitHub distribution, a live remote endpoint, and an active official MCP Registry listing for version 0.2.0. Next experiments, in order
...
2. **Add first-party discovery:** an MRX “Use with your AI assistant” page linked from appropriate learning pages. This requires the website's normal release and SEO checks. Include a plain brow3. **Distribute the working package:** publish the working remote endpoint to the official MCP Registry and submit to relevant client directories. Verify each listing. GitHub presence alone is not registry publication. The remote-only official listing was published and independently read back on September 23, 2026. `docs/server.json` describes that endpoint and does not depend on PyPI. Add a PyPI package entry only after that distribution is publicly verified. The README includes the required package identity marker. Use the [official registry publishing guide](https://modelcontextprotocol.io/registry/quickstart).delcontextprotocol.io/registry/quickstart).
